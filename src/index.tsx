...
// --- Sub-Components (Defined OUTSIDE to fix focus/cursor bug) ---

const DashboardView: React.FC<ViewProps> = ({ data, license, setView }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div className="mt-4 flex items-center text-green-600 text-xs font-medium bg-green-50 w-fit px-2 py-1 rounded">
             <TrendingUp size={14} className="mr-1"/> Revenue
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div className="mt-4 flex items-center text-orange-600 text-xs font-medium bg-orange-50 w-fit px-2 py-1 rounded">
             <TrendingUp size={14} className="mr-1"/> Expenses
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between">
          <div>
           ...
           <div className="mt-4 flex items-center text-blue-600 text-xs font-medium bg-blue-50 w-fit px-2 py-1 rounded">
             <Building2 size={14} className="mr-1"/> Live Balance
          </div>
        </div>
...
const LedgersView: React.FC<ViewProps> = ({ data, setData }) => {
...
const VoucherView: React.FC<ViewProps> = ({ data, setData }) => {
...
const ReportView: React.FC<ViewProps> = ({ data }) => {
  
  const exportToTally = () => {
    // Simplified Tally XML export logic
    ...
    a.href = url;
    a.download = `NAC_TallyExport.xml`;
           ...
           <div className="text-right">
              ...
              <div className="mt-4 space-y-1">
                  ...
                  <div className="flex justify-end gap-2 pt-2 print:hidden">
                     <button 
                       onClick={() => setInvoiceData({...invoiceData, taxType: 'local'})}
                       className={`text-xs px-2 py-1 rounded border ${invoiceData.taxType === 'local' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-500'}`}
                     >
                       Local (CGST+SGST)
                     </button>
                     <button 
                       onClick={() => setInvoiceData({...invoiceData, taxType: 'inter'})}
                       className={`text-xs px-2 py-1 rounded border ${invoiceData.taxType === 'inter' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-500'}`}
                     >
                       Inter-State (IGST)
                     </button>
                  </div>
                  {/* Visible Tax Type Label for Print */}
                  <div className="hidden print:block text-right text-xs font-bold text-gray-400 uppercase mt-1">
                    Supply Type: {invoiceData.taxType === 'local' ? 'Intra-State' : 'Inter-State'}
                  </div>
              </div>
           </div>
        </div>
             ...
             {invoiceData.items.map((item, idx) => (
               <tr key={idx} className="border-b border-gray-100 group">
                 ...
                 <td className="py-3 px-4 text-center">
                   <select
                     className="outline-none bg-transparent text-center appearance-none cursor-pointer font-bold text-gray-600"
                     value={item.gstRate}
                     onChange={e => {
                        const newItems = [...invoiceData.items];
...
const AdminView: React.FC<ViewProps> = ({ data, setData, license }) => {
  const [genKey, setGenKey] = useState('');
  const [clientName, setClientName] = useState('New Client');
  const [duration, setDuration] = useState('1Y'); 
  const fileInputRef = useRef<HTMLInputElement>(null);

  // SECURITY CHECK: Only Master Admin (You) can generate keys
  const isMasterAdmin = license?.key === 'NAC-DEMO-2025-TEST';
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* 1. Company Settings (Visible to EVERYONE) */}
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-100">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500">Edit your Invoice Details here</p>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <div>
                        ...
                        <textarea 
                            className="w-full border border-gray-200 rounded p-2 focus:border-orange-500 outline-none text-xs text-gray-600 font-mono"
                            rows={6}
          ...
          {isMasterAdmin ? (
            <div className="bg-slate-900 text-white p-8 rounded-xl shadow-lg border border-gray-800 h-fit">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h3 className="font-bold text-white text-lg">Master Admin Panel</h3>
                        <p className="text-xs text-gray-400">License Key Generator</p>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input 
                            type="text" 
                            className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 outline-none text-white"
                            value={clientName}
                            onChange={e => setClientName(e.target.value)}
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-400 mb-1">License Duration</label>
                        <select 
                            className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 outline-none text-white"
                            value={duration}
                    ...
                    <button onClick={generateKey} className="bg-orange-600 text-white px-4 py-3 rounded-lg w-full font-bold hover:bg-orange-700 transition shadow-lg mt-2">
                        Generate License Key
                    </button>
                </div>

                {genKey && (
                    <div className="bg-gray-800 p-4 rounded-lg text-center border border-gray-700 mt-6 animate-fade-in">
                        <p className="text-xs text-green-400 font-bold mb-2">KEY GENERATED</p>
                        <p className="text-xl font-mono font-bold text-white select-all cursor-pointer tracking-widest break-all">{genKey}</p>
                        <button 
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        {/* View Content Area - COMPONENTS RENDERED HERE WITH PROPS */}
        <main className="flex-1 overflow-y-auto bg-gray-50 scroll-smooth">
          {view === 'dashboard' && <DashboardView data={data} setData={setData} license={license} setView={setView} />}
          {view === 'ledgers' && <LedgersView data={data} setData={setData} license={license} setView={setView} />}
          {view === 'vouchers' && <VoucherView data={data} setData={setData} license={license} setView={setView} />}
          {view === 'reports' && <ReportView data={data} setData={setData} license={license} setView={setView} />}
          {view === 'invoice' && <InvoiceView data={data} setData={setData} license={license} setView={setView} />}
          {/* Both admin and settings point to AdminView, logic inside handles access control */}
          {(view === 'admin' || view === 'settings') && <AdminView data={data} setData={setData} license={license} setView={setView} />}
        </main>
      </div>
    </div>
  );
